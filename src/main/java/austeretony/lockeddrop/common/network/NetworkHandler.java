package austeretony.lockeddrop.common.network;
...
public class NetworkHandler {
    ...
    public static void init() {
        channel = NetworkRegistry.INSTANCE.newEventDrivenChannel(LockedDropMain.MODID);
    ...
    private static FMLProxyPacket pack(ProxyPacket modPacket) {
        ...
        FMLProxyPacket packet = new FMLProxyPacket(packetBuffer, LockedDropMain.MODID);
        return packet;
    }
    ...
    private void process(FMLNetworkEvent.CustomPacketEvent event) throws IOException {
        ...
        if (LockedDropMain.MODID.equals(proxyPacket.channel())) {
            ...
            if (readableBytes != 0) {
                ...
                if (packet == null) {
                    LockedDropMain.LOGGER.error("Wrong packet id: " + id);
                } else {
